...
function persist(){localStorage.setItem(STORAGE_KEY,JSON.stringify(products));localStorage.setItem(HISTORY_KEY,JSON.stringify(history.slice(0,500)));localStorage.setItem(MEALS_KEY,JSON.stringify(meals.slice(0,300)));renderAll()}
...
$("#saveProductBtn").onclick=()=>{
 ...
 const data={name,brand:$("#productBrand").value.trim(),barcode:$("#productBarcode").value.trim(),location:$("#productLocation").value,qty:Math.max(0,num($("#productQty").value)),minStock:Math.max(0,num($("#productMinStock").value)),expiry:$("#productExpiry").value,image:$("#productImage").src&& !$("#productImage").src.startsWith(location.href)?$("#productImage").src:"",kcal100:num($("#productKcal100").value)||null,protein100:num($("#productProtein100").value)||null,carbs100:num($("#productCarbs100").value)||null,fat100:num($("#productFat100").value)||null,nutritionBasis:$("#productNutritionBasis").value,nutritionSource:$("#nutritionSource").textContent,updatedAt:nowIso()};
...
$$(".stock-btn").forEach(b=>b.onclick=()=>{const p=products.find(x=>x.id===stockId);if(!p)return;const amount=Math.max(1,num($("#moveQty").value)),requested=num(b.dataset.delta)*amount;if(requested<0&&p.qty<=0){toast("No queda stock");return}const oldQty=p.qty;p.qty=Math.max(0,p.qty+requested);const actualDelta=p.qty-oldQty;p.updatedAt=nowIso();addHistory(p,actualDelta,actualDelta>0?"Entrada":"Consumo");$("#stockCurrent").textContent=p.qty;$("#moveQty").value=1;persist();toast(actualDelta>0?`Añadidas ${actualDelta} ud.`:`Retiradas ${Math.abs(actualDelta)} ud.`)});
...
async function fetchProduct(barcode){
 ...
 try{
  const fields="product_name,product_name_es,brands,image_front_small_url,image_front_url,nutriments,serving_quantity_unit,product_quantity_unit";
  ...
  if(data.status===1&&data.product){
    ...
    const unit=(p.serving_quantity_unit||p.product_quantity_unit||"g").toLowerCase();$("#productNutritionBasis").value=unit==="ml"?"ml":"g";$("#nutritionSource").textContent="Open Food Facts";toast(kcal?"Producto y nutrición encontrados":"Producto encontrado · faltan kcal")
...
document.addEventListener("click",e=>{const b=e.target.closest("[data-remove-ingredient]");if(b){draftIngredients.splice(num(b.dataset.removeIngredient),1);renderDraftMeal()}});
...
$("#exportBtn").onclick=()=>{const payload={app:"Mi Despensa",version:2,exportedAt:nowIso(),products,history,meals};const blob=new Blob([JSON.stringify(payload,null,2)],{type:"application/json"}),a=document.createElement("a");a.href=URL.createObjectURL(blob);a.download=`mi-despensa-${new Date().toISOString().slice(0,10)}.json`;a.click();URL.revokeObjectURL(a.href);toast("Copia exportada")};
$("#importInput").onchange=async e=>{const file=e.target.files?.[0];if(!file)return;try{const data=JSON.parse(await file.text());if(!Array.isArray(data.products))throw new Error();if(confirm(`¿Importar ${data.products.length} productos? Sustituirá los datos actuales.`)){products=data.products;history=Array.isArray(data.history)?data.history:[];meals=Array.isArray(data.meals)?data.meals:[];persist();closeModal("settingsModal");toast("Copia restaurada")}}catch{toast("El archivo no es una copia válida")}e.target.value=""};
$("#clearAllBtn").onclick=()=>{if(confirm("¿Borrar inventario, comidas e historial? Esta acción no se puede deshacer.")){products=[];history=[];meals=[];persist();closeModal("settingsModal");toast("Datos eliminados")}};
